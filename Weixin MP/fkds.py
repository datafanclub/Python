...
def fkds(urlprefix, urlsuffix, dataset, tmpprfx, colname, savename):
    aurl = urlprefix + '1' + urlsuffix 
    webpage = urllib.request.urlopen(aurl).read().decode('utf-8')
    ...
    for i in range (1,tp):
        ...
        webpage = urllib.request.urlopen(burl).read().decode('utf-8')
...
savefilen = 'Result'
fkds(urlpre, urlsuf, datafileset, filenameprfx, colnm, savefilen)